...
- Added the ability to proportionally resize all uploaded images based on their longest edge (**Edit > Resize > Resize to > Longest edge**); change the 'Longest edge:' value to whatever length in pixels you want each images longest edge to have (short edge will be scaled to match). 'Longest edge' is the default 'Resize to' method.
...
Some thoughts on future modifications, but do keep in mind that this project is being 'developed' in <abbr title="Don't Hold Your Breath">DHYB</abbr>-mode, so they may never happen ...
